...
| ID | | Deliverable | Done when |
|---|---|---|---|
| **P4.1** | M ✅ | Footnotes: structured model, callers, class rendering, splitting, numbering policy | A `\f` note renders at its intended location with its caller intact, including a note that must split across pages; notes never overlap the last lines of a long column ([spike F-10](../spike/NOTES.md)); numbering restarts per the configured policy rather than running continuously through the book; submarkers `fr`, `ft`, `fq`, `fqa`, `fk`, `fl`, `fv` are structural, not flattened (SCR-003, USFM-002). **Done** — asserted against real pages by reading glyph baselines and sizes out of the PDF, not by eye. F-10's first defect turned out to be two: the first page was typeset on a frameset the footnote steal never reached, and the note area could still grow into a committed column. `\fr` is now printed, which it never was. **Per-page numbering is deliberately not offered** — a caller is set when its paragraph is, before the page it lands on is decided |
| **P4.2** | M ✅ | Cross-references as a distinct type, in the note area | An `\x` reference renders in the configured placement and is styled independently of footnotes; `xo`, `xt`, `xk`, `xq` are represented separately (SCR-004, SCR-005). **Done** — all three MVP placements (note area, inline, under the paragraph), and its own caller sequence, so a page carrying both apparatus reads `1` for a note and `a` for a reference rather than interleaving one count between them |
| **P4.3** | M ✅ | Figures: asset resolution, containment, sizing, missing-image policy | A figure renders from a relative project path with `src`, `alt`, `size`, `loc`, `copy`, and `ref` preserved; raster and vector artwork both place; a path escaping the project directory after canonicalization is a diagnostic — the backend enforces nothing here ([spike F-14](../spike/NOTES.md)); a supplied PDF's page box and inherited font subsets are accounted for; a missing image follows the configured policy (SCR-006, USFM-003). **Done** — the class had wrapped the draw in a `pcall`, so a project naming two figures that did not exist built to `[completed]` and wrote a PDF with two holes in it. Containment is checked lexically and again after canonicalization; format is sniffed from the bytes rather than the extension. The PDF question is answered by measurement: a plate placed as a Form XObject brings its **whole page box** — `/BBox [0 0 432 648]`, margins and folio included — and its **embedded font subsets**, which took a two-font output to four. Neither is refusable, so both are one line in the log |
| **P4.4** | M ✅ | Running heads and folios with live verse ranges | Book name, reference range, and page number appear per the settings; the range is correct on a page whose first verse started earlier and across a column break. **Done** — the mechanism came out of S0.4 and had never been asserted, and writing the test found three defects. Upstream's `first-reference` discards its options, so the default head read `Mark        Mark 1:1–1:10` with the book name twice. A page wholly inside one long verse collected no reference and its head went blank; the last reference of each page is now carried into the next. And the book name came from a running global that is ahead of the page being output, so a two-book document headed its first page with the second book — it comes from the page's own references now. A fourth, found on the way: `\eject` between books is a frame break, which in two columns is a *column*, so books ran together on one page |
| **P4.5** | S ✅ | Independent visibility switches | Chapter numbers, verse numbers, section headings, footnotes, and cross-references can each be hidden without altering USFM, and hiding a number does not remove its anchor (SCR-001, SCR-007). **Done** — the switches were built as they were needed; this is the test. SCR-001 is asserted through the running head, which is the only place the anchor is observable once the number is gone: a page with its verse numbers hidden still reports the verses on it |
| **P4.6** | M ✅ | Section headings `s1`–`s4`, `\d`, `\sp`, `\r`; chapter openings | Each renders with its own resolved style; a heading keeps with the text that follows it rather than sitting alone at a column foot. **Done**, and each half found a defect. The cascade was giving `s3` and `s4` whatever `s2` had, so a translation marking four levels of structure was printed with two — they have sizes of their own now, and the golden files record the old behaviour going away. The keep-with-next was there and did nothing: `\par` pushes `document.parskip` after the heading box, glue straight after a box is a legal breakpoint, and every penalty the class added came after that glue. The fix is ordering — `\novbreak` *before* `\par`, which ends the paragraph itself and leaves a penalty on the queue, so `\par` returns without pushing anything. **And a finding that outlives this item:** the runtime ships DejaVu Serif in regular and bold and nothing else, a request for a face a font has not got is answered with the regular face and no complaint, so `italic = true` is invisible on the default configuration — which is thirteen selectors in the built-in sheet, `\it` and `\add` among them. The heading hierarchy is carried by size for that reason; the packaging half belongs to P6.2 |
| **P4.7** | M ✅ | Lists and tables | `li1`–`li4`, `lim1`–`lim4`, and `tr`/`th#`/`tc#` render with correct indentation and column alignment. **Done**, and neither half was there. `\lim` was an unsupported marker, so an embedded list came out as prose; `list.N` is now `list.li1`–`list.li4` and `list.lim1`–`list.lim4`, matching `poetry.q1` — a rename, and the goldens carry it. An item's indent was leading glue, which moves the first line and nothing else, so a wrapped item returned to the margin and stopped reading as an item; it is `lskip` now. `\lh` and `\lf` came along because a list whose header and footer both warn is half a feature. **Tables were never laid out at all** — the M0 placeholder set each cell after the last with a fixed gap, so the second column began wherever the first column's text happened to end and no two rows agreed. They are measured in two passes now, right-aligned cells set from their right edge, and the padding is kerns rather than glue so a row cannot be broken in half. `\tc1-2` was being flattened to a one-column cell and is carried as a span; note that **`usfm-core` reports `\tc1-2` as not a USFM marker**, which USFM 3.0 says it is, so the column range is recovered downstream from the marker text and the upstream warning is spurious. **Limit, deliberate:** a cell is measured as an hbox and never wraps, because a wrapping cell needs a nested typesetter with its own measure and the tables USFM carries are genealogies. A table too wide for its column says so on the backend's stderr, with both measurements, and runs past the margin where it can be seen |
| **P4.8** | S ✅ | PDF metadata and reference anchors | Title, language, author, and subject appear in PDF properties; verse anchors are carried far enough that destinations and bookmarks are a later minor release rather than a model redesign (PDF-005, SCR-008). **Done, and the anchors are real rather than merely possible**: every book, chapter and verse is a named destination — `JHN.3.16`, the form Paratext already speaks, so a prefix of a verse's name is the name of the thing containing it — and books and chapters are an outline. Emitted outside the numbering guards, so SCR-001 holds here too: hiding a verse number does not remove the place it marks. **Verse anchors are opt-in, and that is a measurement**: one destination per verse cost 15% of the build time and 14% of the file size on a 4,950-verse document, and nothing in this release points at one, so `output.anchors` defaults to `chapter` — which gives the outline a reader actually navigates by for almost nothing — with `verse` and `none` either side of it. **`/Lang` is the one part not delivered.** It belongs in the document catalogue; this SILE's `pdf` binding reaches the information dictionary and nothing else, so there is no way to set it from the class. The language still governs hyphenation and line-breaking, and the document simply does not declare it — one for the packaging work, either as a patched runtime or as an incremental update written after SILE finishes. An unset title is *absent* rather than empty, and specifically is not the folder's name: a path that can reach the output can reach a golden file |
...
| ID | | Deliverable | Done when |
|---|---|---|---|
| **P5.1** | M ✅ | Font resolution: project fonts first, system second, unresolved is blocking | A font absent from the system and from `assets/fonts/` blocks the build with a diagnostic naming the setting that requested it; a project-local `.ttf` renders on a machine where it is not installed (FONT-001, FONT-003). **Pulled forward from M5** — FONT-002 needs a file to read a character map from, and M3 could not honestly be shown to anyone with a non-Latin Gospel until it existed. Resolution order is project, then the backend's own font directory, then the system, which is the order the backend's fontconfig applies |
| **P5.2** | M ✅ | Coverage pre-flight from the font's character map; hyphenation-pattern table | **Coverage done and pulled forward:** a Latin-only font configured for Tamil Scripture produces a blocking error naming the character, its count and a verse, before SILE runs (FONT-002). Per-style checking is moot until a style can carry a font — today there is one body font and it is checked against all the text. **FONT-004 done too**, and not as a pattern table: the backend ships Tamil patterns and they fire, so what decides is the script rather than the availability of patterns. A Tamil book went from 510 hyphens to the 7 in its own source text. **A style can now carry a font**, resolved and reported like the body font, so a style font nobody has is a diagnostic naming the style rather than a nil file inside the shaper. **Still to do:** per-style *coverage*, which needs the codepoint set of the text each selector applies to |
| **P5.3** | M ✅ | Corpus build; PDF structural assertions; vendored pinned test fonts | Page count, page geometry, embedded font list, per-page extracted text, and image presence asserted; the suite does not fail when a system font is updated (PDF-001 – PDF-003, DET-002). **Done for what can be done, and the gap is a font gap.** All thirteen corpus books are attempted through the whole application; four compose — Tamil, Latin, Greek, Cyrillic — and the other ten are stopped by coverage and *only* by coverage, which is the assertion worth having: a second defect hiding behind the font one would now fail the suite. The remaining scripts need fonts this repository does not hold, which is P6.2's licensing decision, not a test to write. Fonts come from the repository rather than the machine, so DET-002 holds. **The overfull-line assertion exists now**, and it found the residue of the defect it was written for: zero glyph runs outside a one-column measure, and in two columns five runs on two lines of the Tamil book — the tails of single words about 164pt wide in a 155pt column. There is no legal break inside a Tamil word and the emergency stretch widens spaces rather than narrowing words, so the typesetter can overhang or leave the column half empty. It overhangs, and the test is a ceiling with the numbers in it. **Break opportunities inside Indic words is the fix** and is a decision about a script. **The trailing-blank-pages defect is gone**: 28,820 characters in one paragraph now set to 26 pages with none blank, where it used to leave five — most likely the `balanced-frames` removal, and there is a test so it stays fixed |
| **P5.4** | M ✅ | Draft builds and selected-book UI | A one-book draft after a single style change completes in a small fraction of a full-Bible build and is visibly marked as a draft (BLD-012, BOOK-004). **Done.** BOOK-004 arrived with the Scripture tab; what is new is the draft. A draft is stamped across the top of every page, in the top margin above the running head so it takes no room from the text, and its size and colour are settled in the class rather than in the style sheet — the one place appearance is deliberately not a publisher's to change, because a sheet that could make the mark invisible could turn a proof into something mistakable for a printed book. **And a draft is written beside the finished PDF, never over it**: `Bible.pdf` gets `Bible-draft.pdf`, so a proof of two books cannot replace a publication of sixty-six — the guarantee is that a draft is unable to name the real file, not that it chooses not to. Draft is an argument rather than a setting: it describes one run, and a project that remembered it was drafting would eventually ship a stamped book. The mark says the book count, and carries no date, because two runs of one build have to produce the same bytes. **The timing half of the acceptance is not asserted and deliberately so** — there is no full Bible here to be a fraction of, and a wall clock in a test is a flake waiting for a slow machine; what is asserted is the mechanism it rests on, that a draft typesets only the books it was given |
| **P5.5** | M ✅ | The build cache, and its five-part invalidation key | Reopening a 66-book project lists books in under 500 ms warm; a change to configuration, styles, marker table, backend version, or application version invalidates the cache ([SRS-REVIEW F14d](SRS-REVIEW.md#f14--smaller-gaps-and-conflicts)). **Done, and the measurement changed what got built.** Opening a synthetic 66-book project — 6.4 MB of real Scripture, every word parsed and normalized — takes **260 ms warm** and 671 ms cold. The bar was already met, so a discovery or parse cache would have bought a quarter of a second at the price of a class of stale-read bug, and there is none. **What is slow is the backend**: 7.9 s for one book of Mark, minutes for a Bible. So the cache is the one that skips *that* — a fingerprint of the emitted document, the backend's argument list, the backend version and the application version, and a build whose fingerprint matches the last one keeps the PDF it already has. Measured at 7.9 s to 0.65 s. `Clean` is beside `Draft` on the bar and overrides it, because the fingerprint is a promise about *this project's* inputs and a build also reads a system font and artwork that may live anywhere (BLD-007). The 500 ms criterion is now a test rather than a hope. **A defect found while testing the cache, and worth more than the cache**: a single control character in a source file — a vertical tab, which is what a bad export leaves behind — reached the emitted XML unescaped, and XML 1.0 forbids those outright, *including as numeric references*. The backend refused the whole document with `not well-formed (invalid token)` and named no book, no line and no character. Filtered now, at the one place there is a finished document rather than at each of the several places a string enters one |
| **P5.6** | S ✅ | Open PDF and Open Output Folder; output-path pre-flight | The finished PDF opens in the platform's own viewer and the containing folder can be revealed (GUI-009). **There is no integrated preview** — GUI-008 is a SHOULD and [ADR-003](adr/003-gui.md#revision-the-preview-is-gone-and-most-of-this-argument-with-it) declines it. The pre-flight is the part that matters: an external viewer holds the output open, so a locked destination is now the ordinary case rather than an edge one, and it must be reported **before** a full-Bible build rather than after it (OUT-001, [SRS-REVIEW F10](SRS-REVIEW.md#f10--build-time-is-the-dominant-fact-of-the-workflow-and-the-srs-does-not-confront-it)) | **Done.** Open folder was already there; the PDF now opens too, from the written path on the build bar rather than from a button of its own — the path is the report and the action is the same thing. It opens `session.output` and not the configured path, so a draft opens the draft. **The pre-flight grew the half that was missing.** It checked whether an existing file was locked, which is the second build's failure; the first build's is a destination under a folder that does not exist or that nothing may write to, and that was found at publish — several minutes into a Bible. A missing folder is now created, because a publisher who writes `out/Bible.pdf` means the folder; a folder that is really a file, or one this account cannot write into, blocks before the backend starts. Writability is decided by trying and cleaning up rather than by reading permission bits, which on Windows say very little about what will actually happen |
| **P5.7** | L ⏳ | Packaging SILE, HarfBuzz, fontconfig, ICU, libtexpdf, and the Lua rock tree for three platforms, on the shape S1 proved | **Proved on Windows, wired for the other two.** An 83.6 MB executable carrying its own typesetter built a PDF from a folder of USFM with no SILE installed and every pointing variable scrubbed from the environment — which is the acceptance criterion, on one platform. `scripts/stage-backend.mjs` is the one place that says what a shipped runtime is: it removes `luasec` and `luasocket` (spike F-16, NFR-004), removes the fontconfig cache, which is 2.9 MB of the *build machine's* font list, and copies this repository's own class over whatever the runtime brought. `.github/workflows/release.yml` runs it on all three, and every job ends by building a PDF rather than by compiling — because a binary that links and cannot set a page is exactly what a packaging mistake produces. **Two defects found in the mechanism.** `build.rs` watched the bundle *variable* and not the bundle, so re-staging a corrected class into the same path and rebuilding produced a binary carrying the previous stage, with no symptom but a version-mismatch error at run time — a release built that way would ship the wrong typesetter silently. And a bundle carrying socket code is now refused at start-up rather than warned about, since the entries are already being walked to compute the cache key and a warning about a property of the binary has nobody left to act on it. **What is left needs the other two machines**: the Linux and macOS jobs are written and have never run |
| **P5.8** | M ✅ | SILE error mapping table; raw log collapsed behind it | Each known backend failure class becomes a BibleCompose diagnostic with the raw text available but collapsed; an unmapped failure still surfaces rather than being swallowed (SILE-007, DIA-005) | **Done.** Eight patterns, every one of them a failure that has actually been seen rather than one that was imagined — a table of speculative failures is a table nobody maintains. Each says three things: what happened, **whose fault it is**, and what to do. Whose fault is the part worth insisting on: most of these are defects in this application, and a publisher told to check their Scripture for a malformed XML document will spend an afternoon on it. Two guarantees hold the shape. The raw tail travels with every mapped diagnostic and the panel already collapses it (DIA-005) — a mapping that hid the evidence would be worse than none, because the evidence is what a bug report is made of. And an unmapped failure still surfaces, with the same tail: the table lists what has been seen and does not decide what is worth reporting. Proved on a real one — a stale class in the runtime cache went from `exited with status 1` and forty lines of Lua trace to *the typesetting class and this application are different versions* |
| **P5.9** | S ✅ | Offline end-to-end test; log hygiene review | A full open-edit-build session succeeds with the network disabled and issues zero requests; the build log contains no Scripture by default (NFR-004, NFR-010) | **Done, and both halves came out better as properties than as reviews.** Offline is asserted against the committed `Cargo.lock` rather than by pulling a plug: every workspace crate except the Tauri shell is walked transitively and must reach no HTTP client. The shell is exempt because `tauri` pulls `reqwest` to serve the window's own assets, and the exemption is itself tested — it must be through the framework and never direct, and if it ever disappears the test says to delete the exemption. The window is fenced instead by `default-src 'self'` and a capability list with two entries, both asserted. So the claim is about the code rather than about one machine on one day. **Log hygiene turned out to be already sound and is now guarded**: a book is built whose text is unmistakable, and every byte the build left behind is read looking for it. Nothing holds it — the working directory is removed on success, leaving only a fingerprint and a page count. The two ways Scripture can reach the disk are both asked for: `keep_intermediates`, which is tested to actually do it so the first test is measuring policy and not an accident, and a failed build, which keeps its log because a failure nobody can read is a failure nobody can fix |
...
| ID | | Deliverable | Done when |
|---|---|---|---|
| **P6.1** | M ⏳ | Installers and code signing, three platforms | **Installers built and verified on Windows; signing is wired and unrun.** `npx tauri build` produces a 29 MB `.msi` and a 19 MB NSIS installer, each carrying a 91.6 MB executable with the typesetter inside it. The bundle targets are written out rather than left as `all`, because `all` is whatever today's Tauri supports on today's runner and a release that quietly stopped producing an `.msi` would look like a successful build. The `.deb` declares no dependencies, which is what ADR-006 means in packaging terms. **No certificate, key, password or thumbprint is in this repository and a test refuses to let one in**; signing is driven entirely by repository secrets, and when they are absent the build produces unsigned artefacts and logs a warning saying so — an unsigned release everybody believed was signed being the failure that warning exists to prevent. [RELEASING.md](RELEASING.md) lists what has to be supplied, why Windows takes a thumbprint rather than a certificate, and how to check afterwards. **What is left is not code**: the certificates are purchased and issued by a third party, and the acceptance criterion — no security warning on first launch — can only be met on a machine that has never run a development build |
| **P6.2** | M ⏳ | Default presets and default fonts | **Presets done; the fonts are the part that is not a coding question.** Three editions ship — standard two-column, a reader's edition, and large print — each a TOML fragment compiled in, and each tested from a bare USFM folder for the four things that make a page unacceptable and can be measured: nothing set outside the measure with the bounds taken from that preset's own margins and swapped for the verso pages, no blank page, the trim size asked for, and the thing the preset is *for* actually happening. Applying one **writes into the project's settings file** rather than becoming a third layer of the cascade: provenance stays honest, the publisher can read and disagree with it a line at a time, and nothing new enters the resolver. What is given up is a preset that keeps evolving — which is the right thing to give up, because a publisher's page should not change when they update the software. **And the silent-substitution gap now has a diagnostic** (FONT-005): a font that resolves but has not got the face a style asked for is reported before the build, once per family, naming the styles. It fires on the shipped configuration today — *DejaVu Serif has no italic face, so the 35 styles that ask for one will each be set in the regular face instead*. A warning rather than an error, because the page still sets and that is precisely why nothing else would notice. **What remains is not code**: the italic and bold-italic of the bundled family, and faces for the ten corpus scripts no shipped font covers, each of which is a redistribution question for the people who own them |
| **P6.3** | L ✅ | Accessibility audit and fixes | The whole application is operable by keyboard; the audit passes at the agreed level; complex-script text entry works in every settings field (NFR-011) | **Audited against the running window, not read.** Three findings, all fixed and all verified in a browser. **Dialogs had every role and none of the behaviour**: `role="dialog"` and `aria-modal` tell assistive technology the rest of the page is inert and tell the *browser* nothing, so Tab walked out of a dialog into the form behind it and closing one dropped focus on `<body>`. One `use:modal` action now moves focus in, keeps it in and gives it back, and the linter refuses a dialog without it — because all three had the roles and none had the behaviour, which is what a lint is for. **The build state was never announced.** The progress bar was the only live region and it fired once per page, which reads a thousand times through a Bible; the state word is the live region now and the bar keeps its `progressbar` role, so the count is there for anyone who asks. **Every free-text field is `dir="auto"`** — an Arabic publication name in a left-to-right field puts the caret and the punctuation in the wrong place, and this application is for Scripture in any script. Complex-script entry was already sound and is now checked: Tamil and Arabic round-trip intact, and every settings field commits on `change` rather than `input`, so nothing rewrites a value mid-composition and no input method is interrupted. Keyboard reordering of books was already there beside the drag handles |
| **P6.4** | M ✅ | Localization scaffolding; English strings extracted | No user-facing string is hard-coded in business logic; a second locale can be added without touching Rust (NFR-012) | **Done for what NFR-012 asks, which is GUI strings.** One catalogue in `src/lib/i18n.ts` holds every word the window shows; a second locale is a `Catalogue` passed to `setLocale`, and the type makes every key mandatory so a partial translation is a compile error rather than a window with three English words left in it. No Rust changes, which is the acceptance criterion. The linter now bans a user-facing literal in a component, because English keeps working when one is added and nothing else would notice. Tab titles, group titles and the fifty-odd style property names stay written beside the structures they belong to, where the form is readable, and are addressable by id through an override map — a locale replaces them without touching the structures. **Diagnostics are not translated and the reason is written into the module**: they come from Rust and interpolate values a template would have to be given, so translating them means carrying the message and its arguments separately, which is a change to the diagnostic model rather than to a catalogue. The stable `code` is the key such a catalogue would use when it comes. One defect found by looking at the running window rather than at the type checker: the catalogue and the label module imported each other, and a cycle evaluated at module scope is a blank window that `svelte-check` and `vite build` both pass |
| **P6.5** | M ✅ | The ten acceptance scenarios, scripted end to end | Scenarios A through J of SRS §16.2 all pass in CI, including the forced-backend-failure and cancel cases; source USFM checksums are unchanged after every one | **All ten pass, and writing them found a defect nothing else would have.** Each starts from a *folder* rather than a fixture, because a fixture skips discovery, `\id` identification and the settings file — three of the things a publisher's first five minutes consists of — and every one of them checks the checksum of every source file it started with (BLD-004). **Scenario H found that a project which cannot be opened could still be built.** A settings file that will not parse produced its diagnostic, the window showed it, and the build then went ahead on the defaults and wrote a PDF: exactly what the scenario says must not happen. The CLI had guarded itself and the window had not, which is the argument for the fix living in neither — `BuildRequest` carries what opening the project found, so the build's own blocking check covers it. Scenario C is adapted and says so: the SRS writes it as a style sheet changing the body font size, and in this application that size is a *setting*, so the style standing for it is the one on `\p`. Two things about reading a typeset page had to be learnt the hard way and are written down where the next person will meet them — a line breaks with a hyphen or without one and a PDF records neither the spaces nor the break, and a **verse number is raised onto a baseline of its own**, so it reads back in the middle of the sentence it numbers |
| **P6.6** | M ⏳ | Documentation, release notes, v1.0 sign-off | **Documentation and traceability done; the sign-off is a decision.** [GUIDE](GUIDE.md) takes a new user from a folder of USFM to a PDF and assumes nothing else, and every claim in it was checked against the running application rather than written from memory — which caught a stale `--output` help text on the way. [TRACEABILITY](TRACEABILITY.md) has all 78 MUSTs: 76 verified by a test, two recorded exceptions with their reasoning, none blank. It is generated by `scripts/traceability.mjs`, CI fails if a requirement has nothing pointing at it or if the table has drifted, and **it found a real gap on its first run** — BLD-003 says the PDF's name is derived from the publication's and it was always `bible.pdf`. The README said *83 tests, no GUI*, which had been untrue for four milestones. **Release notes and the sign-off are not written**: notes for a version that cannot ship are premature while P5.7 and P6.1 wait on other machines and on certificates, and signing off is somebody's judgement rather than somebody's task |

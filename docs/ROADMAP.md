...
| **P5.3** | M | Full 66-book corpus build; PDF structural assertions; vendored pinned test fonts | Page count, page geometry, embedded font list, per-page extracted text, and image presence asserted; the suite does not fail when a system font is updated (PDF-001 – PDF-003, DET-002). **Add an overfull-line assertion**: no glyph origin outside the text block. That is what caught the line-breaking defect [ARCHITECTURE §7.4](ARCHITECTURE.md#74-when-a-line-cannot-be-broken) describes, and nothing in the suite would have — it was found by reading glyph positions out of a PDF by hand. **One assertion is already owed to it**: a single paragraph long enough to run past about ten pages produces trailing blank pages — measured at five blanks from a 28,000-character paragraph, none at 11,000, and it reproduces on the class as it stood before P4.4. No real Scripture paragraph is that long, which is why it has waited, but nothing would catch it |

...
///
/// `%LOCALAPPDATA%\Microsoft\Windows\Fonts`, and `None` when the directory
/// does not exist — fontconfig warns about a directory it cannot read, and a
/// warning in every build log about a folder the user has never used is noise
/// that trains people to ignore the log.
fn user_font_dir() -> Option<Utf8PathBuf> {
    let local = std::env::var("LOCALAPPDATA").ok()?;
    let dir = Utf8PathBuf::from(local).join("Microsoft/Windows/Fonts");
...
/// content is a pure function of `root` and of where this machine keeps fonts,
/// which makes rewriting it harmless.
fn ensure_fontconfig(root: &Utf8Path) -> Option<Utf8PathBuf> {
    ...
    //
    // Windows has offered "Install for me only" since 10 1809, it is the
    // default when you double-click a font file, and it does not need an
    // administrator — so it is where most fonts a publisher adds themselves
    // end up. `WINDOWSFONTDIR` does not cover it: that is `C:\Windows\Fonts`
    // and nothing else. Leaving it out is what made a Tamil font that was
    // plainly installed, that the font pre-flight had already resolved and
    // checked for coverage, die inside the shaper with `attempt to index
    // local 'file' (a nil value)` — a nil font file, several frames from
    // anything that names one.
    //
    // Written literally rather than as a magic token because the fontconfig
    // in this bundle is cross-built and its handling of `WINDOWSUSERFONTDIR`
    // cannot be assumed; an absolute path needs nothing from it.
    let user_fonts = user_font_dir();
    let user_entry = user_fonts
        .as_ref()
        .map(|d| format!("\n  <dir>{d}</dir>"))
        .unwrap_or_default();
...
<fontconfig>
  <dir>{bundled}</dir>
  <dir>WINDOWSFONTDIR</dir>{user_entry}
...
mod tests {
    ...
    /// A font installed for one user only is still an installed font.
    ///
    /// `WINDOWSFONTDIR` is `C:\Windows\Fonts` and nothing else, so leaving
    /// the per-user directory out made every font added by double-clicking it
    /// invisible to the shaper — which surfaced as a nil font file several
    /// frames inside harfbuzz, on a font the pre-flight had already resolved
    /// and checked.
    #[test]
    #[cfg(windows)]
    fn the_generated_fontconfig_looks_where_this_user_installs_fonts() {
        let dir = tempfile::tempdir().expect("tempdir");
        let root = Utf8PathBuf::from_path_buf(dir.path().to_path_buf()).expect("utf8");

        let conf = ensure_fontconfig(&root).expect("Windows gets a generated config");
        let body = std::fs::read_to_string(conf.as_std_path()).expect("written");

        assert!(body.contains("WINDOWSFONTDIR"), "{body}");
        match user_font_dir() {
            Some(user) => assert!(
                body.contains(user.as_str()),
                "the per-user font directory is missing: {body}"
            ),
            // Nothing to list, and listing a directory that is not there only
            // buys a warning in every build log.
            None => assert!(!body.contains("Microsoft/Windows/Fonts"), "{body}"),
        }
    }

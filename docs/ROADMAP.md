...
| **P2.3** | M ◑ | Settings schema, embedded defaults, field-by-field merge, `schema_version` | A USFM-only folder builds from embedded defaults; changing only `page.width` leaves unrelated defaults intact; an unknown schema version is one clear diagnostic (CFG-001, CFG-002, CFG-008). **Resolution done**: `defaults.toml` embedded, merge field-by-field, every value `Sourced`. Still to come: the resolved values reaching the page |

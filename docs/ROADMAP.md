...
| **P5.7** | L ✅ | Packaging SILE, HarfBuzz, fontconfig, ICU, libtexpdf, and the Lua rock tree for three platforms, on the shape S1 proved | **Done, on all three.** One executable per platform, each carrying its own typesetter, each proved by building a PDF rather than by compiling — and the Windows one proved on a machine that did not build it. Eleven runs to get there, and every failure was the same shape: **a dependency that existed, somewhere the step was not looking, reported as something else.** `cannot find Lua includes` was a keg-only Homebrew path; `pkg-config has not been configured to support cross-compilation` was one missing variable in a Rust build script; `Sorry, you need zlib with compress2` was `ld: library 'png' not found`; `module 'vstruct' not found` was three rocks `make` does not leave behind; `the backend did not report a version` was mlua statically linking LuaJIT so the C rocks had no symbols to bind to — which the S1 spike had written down for Windows and nobody had applied to Unix. **Two of those were only findable because the diagnostics were improved first**: SILE-002 now reports how the process ended, and the CLI now prints a diagnostic's detail at all, which it never had. **Still true and still recorded**: the Linux and macOS executables carry SILE and not the system libraries SILE links against, and the runner that builds is the runner that runs — so *fresh machine* is proved on Windows and assumed on the other two |
...
| ID | | Deliverable | Done when |
|---|---|---|---|
| **P6.1** | M ⏳ | Installers and code signing, three platforms | **Installers done on all three; signing is wired, exercised and unsigned.** A tagged build now produces `.msi`, `-setup.exe`, `.dmg`, `.deb` and `.AppImage`, plus one command-line executable per platform and a `SHA256SUMS` covering them, and drafts a release a person publishes. The whole path was checked from the other end: the published Windows artefact was downloaded, verified against its published checksum, and used to set a page. **The last defect was the signing arrangement itself** — the secrets are lifted to the job's environment so steps can test them, and a repository without them got `APPLE_CERTIFICATE=""`, which the bundler read as *sign with this* and failed on a certificate that was never there. Empty is now removed rather than passed on, so an unsigned build is an unsigned build rather than a failed one — which is what the arrangement claimed and did not do. **What is left is the certificates**, which are bought rather than written |
